...
- (BOOL) _uploadData:(NSData*)data withFileName:(NSString *)fileName Folder:(NSString*)folderName toServer:(FMServer*)server{
    BOOL success = YES;
    ...
    NSURL * finalURL = [[server.destination ftpURLForPort:server.port] URLByAppendingPathComponent:[NSString stringWithFormat:@"%@/%@",folderName,fileName]];
    And(success, (finalURL != nil));
    Check(success);
...
- (BOOL) uploadData:(NSData*)data withFileName:(NSString *)fileName Folder:(NSString*)folderName toServer:(FMServer*)server{
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!data) {
    ...
    return RunInSeparateThread([self _uploadData:data withFileName:fileName Folder:folderName toServer:server]);
}
